...
import { SistemasData, EntidadData, Ppempresarial } from '../../interfaces/app.dataSistemas.interface';
...
export class SistemasComponent implements OnInit {
  ...
  public dataSource = new MatTableDataSource<Ppempresarial>();
  public data: any;
  ...
  cargarCentros() {
    this.api.getppempresarial().subscribe((data: any) => {
  ...
  editarCentro(centro: SistemasData) {
    const dialogRef = this.dialog.open(EditsisComponent, {
      width: '350px',
      data: {
        data: centro
      }
  ...
  deleteCentros(index: number, id: number, titulo: string, ) {
    this.index = index;
    this.id = id;
    const dialogRef = this.dialog.open(DelsisComponent, {
      width: '500px',
      data: {
        index: index,
        id: id,
